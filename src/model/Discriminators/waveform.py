...
class DiscriminatorBlock(nn.Module):
    def __init__(self, channels, kernel_sizes, strides, groups, neg_slope):
        ...
        for i in range(len(kernel_sizes)):
            ...
            self.convs.append(
                weight_norm(nn.Conv1d(channels[i], channels[i + 1], kernel_sizes[i], strides[i], groups=groups[i])))
...
class Discriminator(nn.Module):
    def __init__(self, downsamples, avg_ker_sizes, avg_strides, avg_paddings, channels, kernel_sizes, strides, groups,
                 neg_slope):
        super().__init__()
        ...
        for ds in downsamples:
            self.dis_blocks.append(DiscriminatorBlock(channels, kernel_sizes, strides, groups, neg_slope))
    ...
    def forward(self, x):
        ...
        for i in range(len(self.dis_blocks)):
            ...
            if i != len(self.dis_blocks) - 1:
                x = F.avg_pool1d(x, kernel_size=self.avg_ker_sizes[i], stride=self.avg_strides[i], padding=self.avg_paddings[i])
        return xs, features
