...
class ResidualUnit(nn.Module):
    def __init__(self, N, m, s, kernel_size, neg_slope=0.2):
        ...
        self.skip_con = nn.Conv2d(
            kernel_size=(1, 1), in_channels=N, out_channels=m * N, stride=s
        )
...
class STFTDiscriminator(nn.Module):
    def __init__(
        ...
        self.conv1 = nn.Conv2d(
            ...
            channels[0],
            kernel_pre_size,
            padding=(kernel_pre_size[0] // 2, kernel_pre_size[1] // 2),
        )
        self.units = nn.ModuleList()
        for i in range(len(ms)):
            self.units.append(
                ResidualUnit(channels[i], ms[i], strides[i], kernel_in_size, neg_slope)
            )
